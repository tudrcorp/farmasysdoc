    ...
    <section class="fi-farmaadmin-branch-monthly-stats">
        <header class="fi-farmaadmin-branch-monthly-stats__header">
            ...
            <p class="fi-farmaadmin-branch-monthly-stats__description">
                {{ $monthLabel }} · {{ $registered_branches_count }} sucursales · {{ $scope_description }}
            </p>
        </header>

        <div class="fi-farmaadmin-branch-monthly-stats__grid">
            @forelse ($branchCards as $index => $branch)
                @php
                    $variant = $index % 3;
                    $bcvRate = $branch['bcv_rate_used'];
                    $bcvLabel = $bcvRate !== null
                        ? 'Bs. ' . number_format($bcvRate, 2, ',', '.') . '/USD'
                        : '—';
                    $hasGoal = (bool) ($branch['has_goal'] ?? false);
                    $goalUsd = $branch['goal_usd'] ?? null;
                    $progressPercent = $branch['goal_progress_percent'] ?? null;
                    $progressBarWidth = $hasGoal && $progressPercent !== null
                        ? min(100, max(0, $progressPercent))
                        : 0;
                    $bcvFootnote = $branch['total_ves'] > 0
                ...
                <article
                    @class([
                        'fi-farmaadmin-branch-monthly-stat',
                        'fi-farmaadmin-branch-monthly-stat--variant-' . $variant,
                    ])
                ...
                >
                    <header class="fi-farmaadmin-branch-monthly-stat__head">
                        <span class="fi-farmaadmin-branch-monthly-stat__icon" aria-hidden="true">
                            <x-filament::icon icon="heroicon-o-building-storefront" class="size-4" />
                        </span>
                        <div class="fi-farmaadmin-branch-monthly-stat__identity">
                            <h3 class="fi-farmaadmin-branch-monthly-stat__name" title="{{ $branch['branch_name'] }}">
                                {{ $branch['branch_name'] }}
                            </h3>
                        </div>
                    ...
                    <div class="fi-farmaadmin-branch-monthly-stat__goal">
                        <div class="fi-farmaadmin-branch-monthly-stat__goal-meta">
                            @if ($hasGoal && $goalUsd !== null && $progressPercent !== null)
                                <span class="fi-farmaadmin-branch-monthly-stat__goal-label">
                                    Meta {{ Number::currency($goalUsd, 'USD', 'en', 0) }}
                                </span>
                        ...
                        <div
                            ...
                            @if ($hasGoal && $progressPercent !== null)
                                ...
                                aria-label="Progreso de ventas respecto a la meta de {{ $branch['branch_name'] }}"
                            @else
                                aria-valuenow="0"
                                aria-label="Sin meta configurada para {{ $branch['branch_name'] }}"
                            @endif
                        >
                            <div
                    ...
                    <dl class="fi-farmaadmin-branch-monthly-stat__metrics">
                        <div class="fi-farmaadmin-branch-monthly-stat__metric">
                            <dt>USD</dt>
                            <dd>{{ Number::currency($branch['total_usd'], 'USD', 'en', 2) }}</dd>
                        </div>
                        <div class="fi-farmaadmin-branch-monthly-stat__metric">
                            <dt>Bs.</dt>
                            <dd>{{ number_format($branch['total_ves'], 0, ',', '.') }}</dd>
                        </div>
                        <div class="fi-farmaadmin-branch-monthly-stat__metric">
                            <dt>BCV</dt>
                            <dd>{{ $bcvLabel }}</dd>
            ...
            @empty
                <p class="fi-farmaadmin-branch-monthly-stats__empty">
