                ...
                <table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" style="max-width:600px; width:100%; background-color:#ffffff; border-radius:16px; overflow:hidden; border:1px solid #d7e6e6;">
                    ...
                    <tr>
                        <td align="center" style="background-color:#0e5c5f; padding:14px 24px;">
                            <p style="margin:0; color:#d7f3f4; font-size:12px; letter-spacing:1.6px; text-transform:uppercase; font-weight:700;">Comprobante de pago masivo a proveedor</p>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:28px 28px 8px 28px;">
                            <span style="display:inline-block; background-color:#e6f6e8; color:#166534; font-size:12px; font-weight:700; letter-spacing:0.4px; text-transform:uppercase; padding:6px 12px; border-radius:999px;">
                                {{ $invoiceCount }} factura{{ $invoiceCount === 1 ? '' : 's' }} cancelada{{ $invoiceCount === 1 ? '' : 's' }}
                            </span>
                            <h1 style="margin:14px 0 8px 0; font-size:24px; line-height:1.25; color:#0e5c5f;">Pagos registrados a su favor</h1>
                            <p style="margin:0 0 20px 0; font-size:15px; line-height:1.55; color:#4b5c5c;">
                                Farmadoc registró un pago que cancela
                                <strong style="color:#1a1a1a;">{{ $invoiceCount }} factura{{ $invoiceCount === 1 ? '' : 's' }}</strong>
                                de <strong style="color:#1a1a1a;">{{ $supplierName }}</strong>.
                                El listado y el detalle de cada pago van en el PDF adjunto.
                            </p>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td style="padding:0 28px 12px 28px;">
                            <p style="margin:0 0 10px 0; font-size:13px; font-weight:700; color:#0e5c5f; letter-spacing:0.4px; text-transform:uppercase;">Facturas canceladas</p>
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border:1px solid #d7e6e6; border-radius:12px; overflow:hidden;">
                                <tr>
                                    <td style="padding:9px 12px; background-color:#0e5c5f; color:#ffffff; font-size:11px; font-weight:700;">Factura</td>
                                    <td style="padding:9px 12px; background-color:#0e5c5f; color:#ffffff; font-size:11px; font-weight:700;">OC</td>
                                    <td style="padding:9px 12px; background-color:#0e5c5f; color:#ffffff; font-size:11px; font-weight:700;" align="right">USD</td>
                                    <td style="padding:9px 12px; background-color:#0e5c5f; color:#ffffff; font-size:11px; font-weight:700;" align="right">Bs</td>
                                </tr>
                                @foreach ($invoiceLines as $index => $line)
                                    <tr>
                                        <td style="padding:9px 12px; background-color:{{ $index % 2 === 0 ? '#f7fbfb' : '#ffffff' }}; font-size:13px; font-weight:700; color:#1a1a1a; border-bottom:1px solid #e4efef;">{{ $line['invoice'] }}</td>
                                        <td style="padding:9px 12px; background-color:{{ $index % 2 === 0 ? '#f7fbfb' : '#ffffff' }}; font-size:13px; color:#5b6b6b; border-bottom:1px solid #e4efef;">{{ $line['purchase'] }}</td>
                                        <td style="padding:9px 12px; background-color:{{ $index % 2 === 0 ? '#f7fbfb' : '#ffffff' }}; font-size:13px; color:#1a1a1a; text-align:right; border-bottom:1px solid #e4efef;">{{ $line['usd'] }}</td>
                                        <td style="padding:9px 12px; background-color:{{ $index % 2 === 0 ? '#f7fbfb' : '#ffffff' }}; font-size:13px; color:#1a1a1a; text-align:right; border-bottom:1px solid #e4efef;">{{ $line['ves'] }}</td>
                                    </tr>
                                @endforeach
                            </table>
                        </td>
                                ...
                                <tr>
                                    <td style="padding:16px 18px;">
                                        ...
                                        <p style="margin:0; font-size:13px; line-height:1.5; color:#6b5420;">
                                            El archivo <strong>{{ $pdfFilename }}</strong> incluye el listado de facturas y el detalle de cada pago (montos, retención SENIAT y forma de pago).
                                            @if ($hasPaymentProof)
                                                También quedó un comprobante asociado a estas cuentas por pagar.
                                            @endif
                                        </p>
                                    </td>
                                </tr>
                    ...
                    <tr>
                        <td style="background-color:#0e5c5f; padding:18px 28px;">
                            ...
                            <p style="margin:0; color:#c5e7e8; font-size:12px; line-height:1.45;">
                                Este correo se generó de forma automática al registrar el pago masivo. Conserve el PDF como comprobante.
                            </p>
                        </td>
                    </tr>
                </table>
