...
class DeliveryActiveJobsController extends Controller
{
    /**
     * Pedidos y traslados en proceso asignados al repartidor autenticado.
     */
    public function __invoke(Request $request): JsonResponse
    {
        ...
        $orderJobs = $orders->map(static function (Order $order) use ($itemsByOrderId): array {
            ...
            return [
                'kind' => 'order',
                'job_stage' => 'en_proceso',
                'id' => $order->id,
                'sort_at' => $order->updated_at?->toIso8601String(),
        ...
        $transferJobs = $transfers->map(static function (ProductTransfer $transfer): array {
            ...
            return [
                ...
                'job_stage' => 'en_proceso',
                'id' => $transfer->id,
                'sort_at' => $transfer->in_progress_at?->toIso8601String()
                    ?? $transfer->updated_at?->toIso8601String(),
                'code' => $transfer->code,
