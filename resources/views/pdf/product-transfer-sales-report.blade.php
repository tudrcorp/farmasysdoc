...
<head>
    <meta charset="UTF-8">
    <title>Traslados de venta — {{ $from->format('d/m/Y') }} – {{ $until->format('d/m/Y') }}</title>
    <style>
...
<body>
    <div class="header">
        ...
        @endif
        <h1>Reporte de traslados de venta</h1>
        <div class="meta">
    ...
    @forelse ($transfers as $transfer)
        <table class="main">
            ...
            <tr>
                <th>Venta</th>
                <td>{{ $transfer->sale?->sale_number ?? '—' }}</td>
                <th>Cliente</th>
            ...
            <tr>
                <th>Delivery</th>
                <td>{{ $transfer->deliveryUser?->name ?? 'Sin asignar' }}</td>
                <th>Costo envío</th>
                <td class="num">
                    @if ($transfer->total_transfer_cost !== null)
                        ${{ number_format((float) $transfer->total_transfer_cost, 2, '.', ',') }} USD
                    @else
                        —
                    @endif
                </td>
            ...
            @endif
            @if (filled($transfer->delivery_recipient_name) || filled($transfer->delivery_address) || filled($transfer->delivery_recipient_phone))
                <tr>
                    <th>Entrega</th>
                    <td colspan="3">
                        {{ collect([
                            $transfer->delivery_recipient_name,
                            $transfer->delivery_recipient_phone,
    ...
    @empty
        <p>No hay traslados de venta en el período seleccionado.</p>
    @endforelse

    <div class="summary">
        Total documentos: {{ $transfers->count() }} · Líneas de producto: {{ $total_lines }}
        · Suma costos de envío: ${{ number_format($total_cost_sum, 2, '.', ',') }} USD
    </div>
